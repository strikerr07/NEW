...
public class ClientAppInterface {
    ...
    public void readCommands() {
        ...
        while (true) {
            ...
            // if the command begins with sync, synchronize the global index with the local index
            if (command.length() >= 4 && command.substring(0, 4).compareTo("sync") == 0) {
                // TO-DO implement sync operation
                continue;
            }
            ...
            if (command.length() >= 6 && command.substring(0, 6).compareTo("search") == 0) {
                // TO-DO implement search operation
                continue;
            }
